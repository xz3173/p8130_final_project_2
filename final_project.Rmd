...
train_set2<- selected_data[train_indices,]
test_set <- clean_data2[-train_indices, ]
test_set2 <- selected_data[-train_indices, ]
...
cv_object <- cv.glmnet(X, y, family = "binomial", alpha = 0.5, type.measure = "class",nfolds=10, lambda = lambda_seq)
...
#Training set without full variables
...
X <- as.matrix(train_set2[, setdiff(names(train_set2), "status")])  # Predictor variables
y <- train_set$status  # Response variable
...
test_set2 <- test_set2|> select(-status)
...
# Create the ROC curve
roc_curvenet2 <- roc(response = as.matrix(test_set$status), predictor = as.numeric(test_predictions_probElastic2) )
